...
export function executeQuery(
	...
	return (dispatch, getState) => {
		...
		componentList.forEach((component) => {
			...
			// Only clearQueue when queryLog is present i.e after executing the first query
			// because we don't want to clearQueue the URLParams
				...
				if (mustExecuteMapQuery || !isEqual(currentQuery, oldQuery)) {
					...
					if (isMapComponent && mapData[component].query) {
						...
						if (!mapData[component].persistMapQuery) {
							// clearQueue mapQuery if we don't want it to persist
