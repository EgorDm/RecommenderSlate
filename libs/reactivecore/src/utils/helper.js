...
function createBoolQuery(operation, query) {
	...
	if (operation === 'nest') {
		...
		if (Array.isArray(query) && query.length && typeof query[0] === 'function') {
			return query[0](query.length > 1 ? query[1] : match_all)
		} else if(Array.isArray(query) && query.length) {
			resultQuery = query;
			operation = 'must';
		} else {
			resultQuery = match_all;
		}
	}
...
function getQuery(react, queryList) {
	...
	if (Array.isArray(query) && query.length) {
		return query.length === 1 ? query[0] : { bool: { must: query } };
	}
